-- Remove the overly permissive policy that allows anyone to view agency profiles
DROP POLICY IF EXISTS "Anyone can view agency profiles" ON public.profiles;

-- Create a more secure policy that only allows viewing agency contact info 
-- when someone is viewing their published listings
...
USING (
  ...
  AND EXISTS (
    ...
    WHERE listings.agency_id = profiles.id 
    AND listings.status = 'PUBLISHED'
  )
);

-- Create a secure function to get limited agency info for listing displays
-- This function only exposes necessary business information, not personal details
...
-- Update the existing get_listings_with_agency function to use the secure approach
...
RETURNS TABLE(
  ...
  published_at timestamp with time zone, agency_name text, agency_phone text, 
  agency_email text
)
LANGUAGE sql
STABLE
...
AS $$
  SELECT 
    ...
    agency_info.agency_phone,
    agency_info.agency_email
  FROM listings l
  LEFT JOIN LATERAL (
