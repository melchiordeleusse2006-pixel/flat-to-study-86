...
-- Drop existing functions to allow changing their signatures
DROP FUNCTION IF EXISTS public.get_agency_business_info(uuid);
DROP FUNCTION IF EXISTS public.get_listings_with_agency(integer, integer, text, integer, integer, text, integer);

-- Create the updated get_agency_business_info function to only return agency name
...
-- Recreate the get_listings_with_agency function to only include agency name
